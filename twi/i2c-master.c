...
/****************************************************************************/
/**
    @brief   Return status data on the interface.

Upper byte is control register and lower byte is status register

*/

uint16_t twiStatusData(void)
{
  return (inb(TWCR) << 8) + inb(TWSR);
}
/****************************************************************************/
/**
...
If the buffer is full, the byte is not written. This is not a circular buffer.
While it is possible to use such a buffer it will significantly complicate
the code to attempt to ensure data is written fast enough to keep up with the
transaction. The result is that only write transactions no longer than the
buffer length are possible.
...
{
  while (twiState != TWI_IDLE) return FALSE;
  twiLastError = 0;
...
ISR(SIG_TWI)
{
  ...
  if (twiState == TWI_START)
  {
/** START: After the START condition has been transmitted and successfully
indicated, the ADDRESS state is setup for the next interrupt. The device
...
/* Returned status indicates some failure of the bus */
    else
    {
      ...
      outb(TWCR,TWI_STOP);                  /* Force stop the bus transaction */
    }
  }

/** ADDRESS - READ: When the device address is successfully transmitted, prepare for
reading the first data byte */
  else if (twiState == TWI_ADDRESS)
  {
    if (twiTransactionType == READ_TRANSACTION)
    {
...
This is usually due to the device not being present so an error is raised. */
...
/* ADDRESS - WRITE: When the device address is successfully transmitted, prepare
for writing the first data byte */
...
stop as it has no further data to send. The current data item is valid. */
  else if (twiState == TWI_DATA_PHASE)
  {
    if (twiTransactionType == READ_TRANSACTION)
    {
...
/* The device indicates that it cannot provide any more data. This is not
necessarily an error but will indicate end of data block or slow retrieval. */
      else if (twiStatus == MR_DATA_NACK)   /* Device cannot provide more */
      {
        outb(TWCR,TWI_STOP);                /* send Stop condition */
        twiState = TWI_IDLE;
      }
/* Returned status indicates some failure of the bus */
      ...
      {
        twiLastError = twiStatus;           /* DATA transmission error */
        twiState = TWI_IDLE;                /* Abort transaction */
      }
    }
...
/* The device indicates that it cannot receive any more data.  This is not
necessarily an error but will indicate end of data block or slow reception. */
